...
class Node {
    ...
    connectToPeers(newPeers) {
        ...
        let me = `https://${this.host}:${this.port}`;
    ...
    sendPeer(peer, peerToSend) {
        const URL = `https://${peer.url}/node/peers`;
    ...
    getLatestBlock(peer) {
        const URL = `https://${peer.url}/blockchain/blocks/latest`;
        let self = this;
    ...
    getBlocks(peer) {
        const URL = `https://${peer.url}/blockchain/blocks`;
        let self = this;
    ...
    sendTransaction(peer, transaction) {
        const URL = `https://${peer.url}/blockchain/transactions`;
    ...
    getConfirmation(peer, transactionId) {
        ...
        const URL = `https://${peer.url}/blockchain/blocks/transactions/${transactionId}`;        
        console.info(`Getting transactions from: ${URL}`);
        return superagent
            .get(URL)
            .then(() => {
