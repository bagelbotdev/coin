...
class Miner {
        ...
        if (rewardAddress != null) {
            let rewardTransaction = Transaction.fromJson({
                ...
                data: {
                    ...
                    outputs: [
                        {
                            amount: index > 1 ? Config.MINING_REWARD:  Config.INITIAL_MINING_REWARD, // satoshis format
    ...
    /* istanbul ignore next */
    static proveWorkFor(jsonBlock, difficulty, silent) {
        ...
        if (!silent) console.info(`Block found: time '${process.hrtime(start)[0]} sec' dif '${difficulty}' hash '${block.hash}' nonce '${block.nonce}'`);
        return block;
    }
}
