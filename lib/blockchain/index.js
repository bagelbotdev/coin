...
class Blockchain {
    constructor(dbName) {
        ...
        this.ordersDb = new Db('data/' + dbName + '/' + )
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        // Check if the sum of output transactions are equal the sum of input transactions + MININaivecoinNG_REWARD (representing the reward for the block miner)
        let sumOfInputsAmount = R.sum(R.flatten(R.map(R.compose(R.map(R.prop('amount')), R.prop('inputs'), R.prop('data')), newBlock.transactions))) + (Config.INITIAL_MINING_REWARD || Config.MINING_REWARD);
