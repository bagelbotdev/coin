...
class Blockchain {
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        let sumOfInputsAmount = R.sum(R.flatten(R.map(R.compose(R.map(R.prop('amount')), R.prop('inputs'), R.prop('data')), newBlock.transactions))) + (index > 1 ? Config.INITIAL_MINING_REWARD : Config.MINING_REWARD);
